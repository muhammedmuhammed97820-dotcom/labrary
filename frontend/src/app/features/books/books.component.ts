...
export class BooksComponent {
  ...
  toggleFavorite(book: Book): void {
    ...
    this.http.post<FavoriteResponse>(`http://localhost:5000/api/auth/favorites/${id}/toggle`, {}).subscribe({
      next: r => { this.auth.updateUser({ ...this.auth.currentUser!, favorites: (r.favorites || []).map(String) }); this.favoriteMessage = r.favorite ? 'تمت إضافة الكتاب إلى المفضلة ✓' : 'تمت إزالة الكتاب من المفضلة ✓'; this.favoriteBusy.delete(id); setTimeout(() => { if (this.favoriteMessage) this.favoriteMessage = ''; }, 2500); },
      error: () => { this.favoriteMessage = 'تعذر تحديث المفضلة. حاول مرة أخرى.'; this.favoriteBusy.delete(id); }
    });
  }
