...
export class BooksComponent implements OnInit {
  ...
  loadBooks(): void {
    ...
    this.http.get<any>('http://localhost:5000/api/books').subscribe({
      next: response => {
        const all: Book[] = Array.isArray(response) ? response : (response?.books ?? response?.data ?? []);
        this.books = Array.isArray(all) ? all : [];
        this.applyFilter();
        this.loading = false;
        this.cdr.detectChanges();
      },
  ...
  private applyFilter(): void {
    ...
    this.filtered = !q ? [...this.books] : this.books.filter(book => `${book.title ?? ''} ${this.author(book)} ${this.category(book)}`.toLowerCase().includes(q));
  }
  ...
  category(book: Book): string { return typeof book.category === 'string' ? book.category : book.category?.name ?? 'عام'; }
  ...
  toggleFavorite(book: Book): void {
    ...
    if (!id || !this.auth.isLoggedIn || this.favoriteBusy.has(id)) return;
    this.favoriteBusy.add(id);
    this.favoriteMessage = '';
    this.cdr.detectChanges();
    this.http.post<FavoriteResponse>(`http://localhost:5000/api/auth/favorites/${id}/toggle`, {}).subscribe({
      next: response => {
        this.auth.updateUser({ ...this.auth.currentUser!, favorites: (response.favorites || []).map(String) });
        this.favoriteMessage = response.favorite ? 'تمت إضافة الكتاب إلى المفضلة ✓' : 'تمت إزالة الكتاب من المفضلة ✓';
        this.favoriteBusy.delete(id);
        this.cdr.detectChanges();
