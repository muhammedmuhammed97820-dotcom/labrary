...
interface Book { _id?: string; id?: string|number; title?: string; description?: string; coverImage?: string; coverUrl?: string; cover?: string; author?: {name?:string;_id?:string}|string; category?: {name?:string;_id?:string}|string; views?:number; viewsCount?:number; downloads?:number; publishedYear?:number|string; isAvailable?:boolean; }
...
export class BooksComponent implements OnInit {
 private readonly http=inject(HttpClient); private readonly auth=inject(AuthService); private readonly api=inject(BookApiService); private readonly notify=inject(NotificationService); private readonly cdr=inject(ChangeDetectorRef); private readonly route=inject(ActivatedRoute); public readonly themeService=inject(ThemeService);
 ...
 getBookCover(book:any){const cover=book?.coverImage||book?.coverUrl||book?.cover;if(!cover)return null;if(/^data:|^blob:|^https?:\/\//i.test(cover))return cover;return this.api.getFileUrl(cover)}
