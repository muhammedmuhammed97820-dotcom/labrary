...
export class BooksComponent implements OnInit {
  ...
  private readonly http = inject((await import('@angular/common/http')).HttpClient);
  ...
  loadBooks(): void {
    ...
    this.api.getAll(this.page, this.limit, this.search.trim()).subscribe({
      next: response => {
        const payload = response as unknown as CatalogResponse | CatalogBook[];
  ...
  private applyFilter(): void {
    ...
    this.filtered = !query
      ? [...this.books]
      : this.books.filter(book => `${book.title ?? ''} ${this.author(book)} ${this.category(book)}`.toLowerCase().includes(query));
  }
  ...
  toggleFavorite(book: CatalogBook): void {
    const bookId = this.id(book);
    if (!bookId || !this.auth.isLoggedIn || this.favoriteBusy.has(bookId)) {
      if (!this.auth.isLoggedIn) this.favoriteMessage = 'سجّل الدخول أولًا لإضافة الكتاب إلى المفضلة.';
      return;
    }
