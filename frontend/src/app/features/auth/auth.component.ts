  ...
  submit(): void {
    ...
    this.http.post<AuthResponse>(`http://localhost:5000/api/auth${endpoint}`, body).subscribe({
      next: response => { 
        ...
        setTimeout(() => this.router.navigateByUrl(response.user.role === 'admin' ? '/admin' : '/books'), 700); 
      },
      error: err => { 
        this.loading = false; 
        const message = err?.error?.message || err?.error?.error; 
        this.error = message || (isLogin ? 'تعذر تسجيل الدخول. تحقق من البريد الإلكتروني وكلمة المرور.' : 'تعذر إنشاء الحساب. حاول مرة أخرى.'); 
        this.cdr.detectChanges();
      }
    });
  }
