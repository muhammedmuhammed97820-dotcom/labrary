...
async function createBook(req, res) {
  try {
    ...
    const book = await Book.create({
      title: title.trim(), author: authorDocument._id, category: categoryDocument._id,
      description: description || "", publishedYear: publishedYear ? Number(publishedYear) : undefined,
      rating: rating ? Number(rating) : 0,
      filePath: `/uploads/books/${bookFile.filename}`, coverImage: `/uploads/covers/${coverFile.filename}`
    });

    const populatedBook = await Book.findById(book._id).populate("author").populate("category");
    res.status(201).json({ message: "Book created successfully.", book: populatedBook });
  } catch (error) {
    console.error(error);
...
async function incrementViews(req, res) {
  try {
    const book = await Book.findById(req.params.id).select("viewsCount");
    if (!book) return res.status(404).json({ message: "Book not found." });
...
module.exports = { getBooks, getBook, createBook, updateBook, deleteBook, incrementViews, incrementDownloads };
