...
function serializeBook(book) {
  ...
  if (!value.author && value.submittedAuthorName) value.author = { name: value.submittedAuthorName };
  if (!value.category && value.submittedCategoryName) value.category = { name: value.submittedCategoryName };
  return value;
}
...
async function getBooks(req, res) {
  try {
    ...
    if (search) filter.$or = [{ title: { $regex: search, $options: "i" } }, { description: { $regex: search, $options: "i" } }, { submittedAuthorName: { $regex: search, $options: "i" } }, { submittedCategoryName: { $regex: search, $options: "i" } }];
    const [books, total] = await Promise.all([
      Book.find(filter).populate("author").populate("category").sort({ createdAt: -1 }).skip((page - 1) * limit).limit(limit),
      Book.countDocuments(filter)
    ]);
    res.json({ books: serializeBooks(books), pagination: { page, limit, total, totalPages: Math.ceil(total / limit), hasNextPage: page * limit < total, hasPreviousPage: page > 1 } });
...
async function createBook(req, res) {
  ...
  try {
    ...
    const book = await Book.create({ title: title.trim(), author: authorId, category: categoryId, submittedAuthorName: isAdmin ? "" : author.trim(), submittedCategoryName: isAdmin ? "" : category.trim(), description: description || "", publishedYear: publishedYear ? Number(publishedYear) : undefined, rating: isAdmin && rating ? Number(rating) : 0, fileId, coverImageId, filePath: "", coverImage: "", status: isAdmin ? "approved" : "pending", submittedBy: req.user._id, rejectionReason: "", reviewedAt: isAdmin ? new Date() : null });
    if (!isAdmin) {
      const admins = await require('../models/user.model').find({ role: 'admin' }).select('_id').lean();
      if (admins.length) await Notification.insertMany(admins.map(admin => ({ user: admin._id, type: 'book_submitted', title: 'طلب كتاب جديد', text: `المستخدم ${req.user.name} أرسل كتاباً جديداً للمراجعة: ${book.title}`, link: '/admin/book-submissions' })));
    }
...
async function updateBook(req, res) {
  ...
  try {
    ...
    if (req.files?.bookFile?.[0]) { const file = req.files.bookFile[0]; newFileId = await uploadBuffer(file.buffer, file.originalname, file.mimetype, { type: "book-file" }, "libraryBooks"); book.fileId = newFileId; book.filePath = ""; }
    if (req.files?.coverImage?.[0]) { const file = req.files.coverImage[0]; newCoverImageId = await uploadBuffer(file.buffer, file.originalname, file.mimetype, { type: "book-cover" }, "libraryCovers"); book.coverImageId = newCoverImageId; book.coverImage = ""; }
    await book.save();
...
async function deleteBook(req, res) {
  try {
    const book = await Book.findById(req.params.id);
    if (!book) return res.status(404).json({ message: "Book not found." });
    if (book.fileId) await deleteFile(book.fileId, "libraryBooks").catch(() => {});
    if (book.coverImageId) await deleteFile(book.coverImageId, "libraryCovers").catch(() => {});
    await book.deleteOne();
    res.json({ message: "Book deleted successfully." });
  } catch (error) { console.error(error); res.status(500).json({ message: error.message || "Failed to delete book." }); }
}

async function getMySubmissions(req, res) {
  try { const books = await Book.find({ submittedBy: req.user._id }).populate("author").populate("category").sort({ createdAt: -1 }); res.json(serializeBooks(books)); }
...
async function getPendingBooks(req, res) {
  try { const books = await Book.find({ status: "pending" }).populate("author").populate("category").populate("submittedBy", "name email").sort({ createdAt: 1 }); res.json(serializeBooks(books)); }
...
async function reviewBook(req, res) {
  try {
    ...
    await book.save();
    if (book.submittedBy) {
      await Notification.create({ user: book.submittedBy, type: status === 'approved' ? 'book_approved' : 'book_rejected', title: status === 'approved' ? 'تمت الموافقة على كتابك' : 'تم رفض كتابك', text: status === 'approved' ? `تمت الموافقة على كتابك «${book.title}» ونشره في المكتبة.` : `تم رفض كتابك «${book.title}». السبب: ${book.rejectionReason}`, link: status === 'approved' ? `/books/${book._id}` : '/my-submissions' });
    }
    const populatedBook = await Book.findById(book._id).populate("author").populate("category").populate("submittedBy", "name email");
...
async function streamBookFile(req, res, mode = "read") {
  try {
    ...
    if (mode === "read" && contentType !== "application/pdf" && !String(file.filename).toLowerCase().endsWith(".pdf")) return res.status(415).json({ message: "This reader currently supports PDF books only." });
    const size = Number(file.length);
    const range = req.headers.range;
    res.setHeader("Accept-Ranges", "bytes");
    res.setHeader("Cache-Control", "private, max-age=300");
    res.setHeader("Content-Type", contentType);
    if (mode === "download") { const safeTitle = String(book.title || "book").replace(/[<>:\"/\\|?*\x00-\x1F]/g, "_").slice(0, 180); res.setHeader("Content-Disposition", `attachment; filename="${safeTitle}.pdf"`); }
    if (range) {
      ...
      if (start >= size || end >= size || start > end) { res.setHeader("Content-Range", `bytes */${size}`); return res.status(416).end(); }
      const chunkSize = end - start + 1;
      res.status(206); res.setHeader("Content-Range", `bytes ${start}-${end}/${size}`); res.setHeader("Content-Length", chunkSize);
