...
async function getAdminBooks(req, res) {
  try {
    ...
    res.json(books);
  } catch (error) { console.error("Admin books load error:", error); res.status(500).json({ message: "Failed to load admin books." }); }
}
...
async function createBook(req, res) {
  try {
    ...
    const bookFile = req.files.bookFile[0]; const coverFile = req.files.coverImage[0]; const isAdmin = canModerate(req);
    let authorId = null; let categoryId = null;
    if (isAdmin) { authorId = (await findOrCreateAuthor(author.trim()))._id; categoryId = (await findOrCreateCategory(category.trim()))._id; }
    const book = await Book.create({ title: title.trim(), author: authorId, category: categoryId, submittedAuthorName: isAdmin ? "" : author.trim(), submittedCategoryName: isAdmin ? "" : category.trim(), description: description || "", publishedYear: publishedYear ? Number(publishedYear) : undefined, rating: isAdmin && rating ? Number(rating) : 0, filePath: `/uploads/books/${bookFile.filename}`, coverImage: `/uploads/covers/${coverFile.filename}`, status: isAdmin ? "approved" : "pending", submittedBy: req.user._id, rejectionReason: "", reviewedAt: isAdmin ? new Date() : null });
    const populatedBook = await Book.findById(book._id).populate("author").populate("category").populate("submittedBy", "name email");
    res.status(201).json({ message: isAdmin ? "Book created and published successfully." : "Book submitted successfully and is awaiting admin review.", book: populatedBook });
  } catch (error) { console.error(error); res.status(500).json({ message: error.message || "Failed to create book." }); }
}
...
async function updateBook(req, res) {
  ...
  } catch (error) { console.error(error); res.status(500).json({ message: error.message || "Failed to update book." }); }
}
...
async function deleteBook(req, res) {
  try {
    ...
    if (!book) return res.status(404).json({ message: "Book not found." });
    await Promise.all([BookView.deleteMany({ book: book._id }), BookDownload.deleteMany({ book: book._id })]);
    res.json({ message: "Book deleted successfully." });
  } catch (error) { console.error(error); res.status(500).json({ message: "Failed to delete book." }); }
}

async function getMySubmissions(req, res) {
  try { const books = await Book.find({ submittedBy: req.user._id }).populate("author").populate("category").sort({ createdAt: -1 }); res.json(books); }
...
async function getPendingBooks(req, res) {
  try { const books = await Book.find({ status: "pending" }).populate("author").populate("category").populate("submittedBy", "name email").sort({ createdAt: 1 }); res.json(books); }
...
async function reviewBook(req, res) {
  try {
    ...
    if (status === "approved") {
      ...
      book.submittedAuthorName = ""; book.submittedCategoryName = "";
    }
    book.status = status; book.reviewedAt = new Date(); book.rejectionReason = status === "rejected" ? String(rejectionReason).trim() : "";
    await book.save();
    const populatedBook = await Book.findById(book._id).populate("author").populate("category").populate("submittedBy", "name email");
    res.json({ message: status === "approved" ? "Book approved and published." : "Book rejected.", book: populatedBook });
  } catch (error) { console.error("Review book error:", error); res.status(500).json({ message: error.message || "Failed to review book." }); }
}
...
async function incrementDownloads(req, res) {
  try {
    ...
    if (!viewerId || viewerId.length > 128) return res.status(400).json({ message: "A valid viewer identifier is required." });
    let counted = false;
    try { await BookDownload.create({ book: book._id, viewerId }); counted = true; }
...
async function downloadBook(req, res) {
  try {
    ...
    if (!book?.filePath) return res.status(404).json({ message: "Book file not found." });
    const relativePath = String(book.filePath).replace(/^[/\\]+/, "");
    const filePath = path.resolve(__dirname, "../../..", relativePath);
    const uploadsRoot = path.resolve(__dirname, "../../..", "uploads", "books");
    if (!filePath.startsWith(uploadsRoot + path.sep) && filePath !== uploadsRoot) return res.status(400).json({ message: "Invalid book file path." });
    if (!fs.existsSync(filePath)) return res.status(404).json({ message: "Book file not found on server." });
    const safeTitle = String(book.title || "book").replace(/[<>:"/\\|?*\x00-\x1F]/g, "_").trim() || "book";
    res.download(filePath, `${safeTitle}.pdf`);
  } catch (error) { console.error("Download book error:", error); res.status(500).json({ message: "Failed to download book." }); }
}

module.exports = { getBooks, getAdminBooks, getBook, createBook, updateBook, deleteBook, getMySubmissions, getPendingBooks, reviewBook, incrementViews, incrementDownloads, downloadBook };
